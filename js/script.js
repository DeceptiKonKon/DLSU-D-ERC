    ...
    if (window.location.pathname.endsWith('dashboard.html')) {
        ...
        document.getElementById('toggle-slider').addEventListener('click', function () {
            ...
            if (slider.style.right === '0px') {
                slider.style.right = window.innerWidth <= 768 ? '-100%' : '-100%'; // Adjust for screen size
            } else {
                slider.style.right = '0px'; // Show slider
            }
        });
        $(document).ready(function() {
            // Fetch the initial data and populate the table and graphs
            refreshGraphs();
            ...
            $('body').on('submit', '.update-form', function(e) {
                ...
                $.post(actionUrl, form.serialize(), function(response) {
                    // After the update, refresh the graphs and table
                    refreshGraphs();
                });
            });
        });
        ...
        function refreshGraphs() {
            $.get('https://gtx50.pythonanywhere.com/get_graph_data', function(data) {
    ...
    if (window.location.pathname.endsWith('viewprotocol.html')) {
        ...
        const username = sessionStorage.getItem('userName');
            ...
            if (accountType == 'erc-chair') {
                ...
                function refreshGraphs() {
                    $.get('https://gtx50.pythonanywhere.com/get_graph_data', function(data) {
                        console.log(data);  // Log the entire response data to check if `title_cluster_graph` is included
...
function getOptionsForSortType(sortType) {
    const optionsMap = {
        ...
        'ethics-status': ['To Process','Not Eligible', 'Exempted', 'Assigning Reviewer', 'Ongoing Review', 'Revision Required', 'Completed'],
